...
ENGINE = MyISAM
COMMENT = 'Armazenamento das formas de pagamento dos clientes.';
...
CREATE TABLE IF NOT EXISTS `Ecommerce`.`FormasDePagamento` (
  ...
  PRIMARY KEY (`idFP`))
ENGINE = MyISAM
COMMENT = 'Armazena as formas de pagamento válidas.';
...
CREATE TABLE IF NOT EXISTS `Ecommerce`.`HistoricoPedido` (
  ...
  `Metadado` VARCHAR(128) NOT NULL COMMENT 'Armazena meta-dados do histórico, tais como código de aprovação do pagamento, código de barras de boleto, código único pix ou o código de rastreio em uma transportadora ou correios.')
ENGINE = MyISAM
